// status valgrind zero error , word 'catch'
...
//scan.c
bool load(const char *dictionary) // compress version to debug
{
    root = malloc(sizeof(node));    if (root == NULL)   {return false;   }
    root->is_word = false;  for (int i = 0; i < N; i++) {root->children[i] = NULL;   }
    // Open dictionary
    FILE *file = fopen(dictionary, "r");    if (file == NULL)   {unload();  return false;   }
    char word[45 + 1];  //LENGTH = 45 Buffer for a word

    while (fscanf(file, "%s", word) != EOF) // Insert words into trie
    {   node *ptr = root;
        for ( int i = 0, l = strlen(word); i<l; i++)
        {   // n - 97 integer of alphabet, which coressponding to children[n]
            int alpha = (int)word[i] - 97; if (alpha == -58) { alpha = 26;} //  apostrophes, 39,

            // Allocate space for new node
            node *n = malloc(sizeof(node));
            if (!n) {return 1; }
                n->is_word = false; for (int j = 0; j < N; j++) { n->children[j] = NULL; }

            n->is_word = false; n->children[alpha] = NULL;  // Add alpha to list

            if (!ptr->children[alpha])  // set this pointer to true (1) for every node reached
            {   ptr->children[alpha] = n;
            }
            ptr = ptr->children[alpha]; // next ptr
        }
        ptr->is_word = true;    // if at end of word, set is_word to true
    }
