...
bool unload(void)
{
    node *ml[46];  int ma[46];
    node *ptr = root;
    int mc = 0; int a = 0;

    ml[0] = ptr;  ma[2] = a; a=2; // for 'c'
    ptr = ptr->children[a]; printf("**** %c\n", a+97);

    ml[1] = ptr;  ma[0] = a; a=0; // for 'a'
    ptr = ptr->children[a]; printf("**** %c\n", a+97);

    ml[2] = ptr;  ma[19] = a; a=19; // for 'c'
    ptr = ptr->children[a]; printf("**** %c\n", a+97);

    // Free memory


        node *next = ml[0]->children[2]; // -> 'c'
        free(ml[0]);
        root = next;
