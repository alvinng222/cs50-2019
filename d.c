...
root level,
pointer
    scan thru the trie for any element in row
    if found element
        check if end of word
        if children not NULL go into this pointer
    next letter and not end of row
...
unsigned int size(void)
{
    ...
    int a = 0; // index for alpha a to z
    node *tp = root;
    int ta;

    for (node *ptr = root; ptr != NULL; ptr = ptr->children[a])
    {
        ...
        test++ ; printf(" %c\n", (a+97)); if (test >50){printf("break!\n"); break;}  // *****
    }
    return n;
}

/*
    //for loop from roots to ptr->next
    for (node *x = ptr; x!=NULL; x = ptr->children[a++])
    //for (node *x = ptr->children[a]; x!=NULL; x = ptr->children[a++])
    {
        printf("a:%i ",a);
        //x = x->children[a];
...
/*{
    ...
    for (int c=0; c<length; c++)    //printf("here we are: %s\n", word);
    {   {alpha = (int)word[c] +32 - 97;}
        ...
        ptr = ptr -> children[alpha]; // pointer to next letter
    }
