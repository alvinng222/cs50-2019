...
bool load(const char *dictionary)
{
    ...
    { if (root->children[i]) printf("* "); else printf(". ");  } printf("\n");
    ...
    while (fscanf(file, "%s", word) != EOF)
    {
        ...
        *** check the value at childen[i] ???
        ...
        for (int i = 0, l = strlen(word); i<l; i++) // i0 is at root
        {
            ...
            //alphebet cp -97 = position of node->children ,cp
            int cp = (int)word[i] - 97;
            //printf("%c %i |", word[i], cp);
            ...
            if (root)
            {
                for (node *ptr = root; ptr != NULL; ptr = ptr->children[cp])
                {
                    if (!ptr->children[cp])
                    {
                        ptr->children[cp] = n;
