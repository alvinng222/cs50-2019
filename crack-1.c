...
bool decrypt(char *h, char *sa)
{
    ...
    for (int keylgth = 2; keylgth <= keyword_lgth; keylgth++)
    {
        ...
        while (bits < max_bits)
        {
            ...
            if (c0 >= al )
            {
                c1 = c0 / al; t[1] = ap[c1 % al];
            }
            
            if (c1 >= al)
            {
                c2 = c1 / al; t[2] = ap[c2 % al];
            }
            
            if (c2 >= al )
            {
                c3 = c2 / al; t[3] = ap[c3 % al];
            }
            
            if (c3 >= al )
            {
                c4 = c3 / al; t[4] = ap[c4 % al];
            }
...
$ ./crypt AAAAA hi
key: AAAAA | salt: hi | crypt: hiYr2cskji5PQ > too long
 ./crypt zzzz 99
