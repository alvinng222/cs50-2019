/* status: load fread fscanf 20 words, still same timing!
    alpha to a
...
bool load(const char *dictionary)
{
    ...
    *temp = *root;
    ...
    while (fscanf(file, "%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s",
    wn[0], wn[1], wn[2], wn[3], wn[4], wn[5], wn[6], wn[7], wn[8], wn[9],
    wn[10], wn[11], wn[12], wn[13], wn[14], wn[15], wn[16], wn[17], wn[18], wn[19]) != EOF)
    {
        for (int q = 0; q<iN; q++)
        {
            //printf("%s ", wn[q]);
            //int a, w;
            node *ptr = root;
            for ( int i = 0, l = strlen(wn[q]); i<l; i++)    // timing same, if change to while
            {
                // convert letter for 27 chilren positions, n - 97 integer of alphabet
                //w = (int)wn[q][i];
                //if ( w == 39 ){ alpha = 26;} else { alpha = w - 97;}  // same time
                int a = (int)wn[q][i] - 97;
...
/*****
    ...
    while (fscanf(file, "%s", word) != EOF)
    {
            //int a, w;
            node *ptr = root;
            for ( int i = 0, l = strlen(word); i<l; i++)    // timing same, if change to while
            {
                // convert letter for 27 chilren positions, n - 97 integer of alphabet
                //w = (int)word[i];
                //if ( w == 39 ){ alpha = 26;} else { alpha = w - 97;}  // same time
